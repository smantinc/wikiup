...
public class ActionUtil {
    ...
    private static void doActionList(ServletProcessorContext context, Document node) {
        if(node.getName().equals("foreach")) {
            ModelProvider mc = context.getModelContainer(Documents.getAttributeValue(node, "in"), context.loadContextAttributes(node));
            String key = Documents.getAttributeValue(node, "key", null);
            Iterable<?> iterable = mc != null ? mc.getModel(Iterable.class) : Null.getInstance();
